...
@section('content')
                    ...
                    <div class="col-lg-4 w-100 col-md-6" >
                        <div class="destinations-three__single">
                            ...
                            <div class="destinations-three__content">
                                <h3><a href="{{ route('shop.show',$categories->id)}}">{{$categories->category_name}}</a></h3>
                            </div>
                            <div class="destinations-three__hover-content">
                                <h3><a href="{{ route('shop.show',$categories->id)}}">{{$categories->category_name}}</a></h3>
                                <p>{{$categories->product->count(). ' Products'}}</p>
                                <a href="{{ route('shop.show',$categories->id)}}  " class="destinations-three__link"><i class="tripo-icon-right-arrow"></i></a>
                            </div>
                        </div>
                    </div>
                    @endforeach
        ...
        <section class="cta-one cta-one__home-two">
            <div class="container h4container">
                <h4 class="h4Content">Second Hand is the safe and simple way to sell <br> pre-loved pieces to buy something new.</h4>
                    <a href="{{ route('shop.index') }}" class="thm-btn cta-one__btn">Start Selling!</a><!-- /.thm-btn cta-one__btn -->
            </div>
        </section>
                    ...
                    <div class="item">
                        ...
                        <div class="tour-one__single">
                            <div class="tour-one__image">
                                <img style="min-width:35wh; max-height:30vh" src="{{asset("sellerproducts/".$products->image)}}" alt="">
                            </div>

                            <div class="tour-one__content" style="min-height: 350px">
                                <ul  class="list-unstyled blog-one__meta">
                                    <li><a href="{{route('seller',$products->seller->id)}}"><i></i> {{"Seller: " . $products->seller->name}}</a></li>
                                </ul>

                                <h3><a href="{{ route('product-details.show',$products->id)}}">{{$products->name}}</a></h3>
                                <p><span>{{$products->price . "JD"}}</span></p>
                                <p><span>{{"Size: ".$products->size}}</span></p>
                                <ul class="tour-one__meta list-unstyled">
                                    <li><a href="{{route('shop.show',$products->category->id)}}"><i class="far fa-list-alt"></i>{{$products->category->category_name}}</a></li>
                                </ul>
                            </div>
        ...
        <section class="cta-four img-responsive" style="background-image: url(assets/images/gallery/secondhandbackgound.jpg);">
            <div class="container">
                ...
                <p>When you buy presolved clothed, you do not add chemicals or prodiction stress to the environment. Everything else is various shades of greenwashing</p>
            </div>
        </section>
                    ...
                    <div class="col-lg-4 wow fadeInUp" data-wow-duration="1500ms" data-wow-delay="000ms" >
                        <div class="blog-one__single" >
                            <div class="blog-one__image">
                                <img style="min-width:35wh; max-height:30vh" src="{{asset("sellerproducts/".$productDate->image)}}" alt="">
                                <a href="{{ route('product-details.show',$productDate->id)}}"><i class="fa fa-long-arrow-alt-right"></i></a>
                            </div>
                            <div class="blog-one__content" style="min-height:280px">
                                <ul class="list-unstyled blog-one__meta">
                                    <li><a href="{{route('seller',$productDate->seller->id)}}"><i></i> {{"Seller: " . $productDate->seller->name}}</a></li>
                                </ul>
                                <h3><a href="{{ route('product-details.show',$productDate->id)}}">{{$productDate->name}}</a></h3>
                                <h6>Size: {{$productDate->size}}</h6>
                                <ul class="tour-one__meta list-unstyled">
                                    <li><a href="{{ route('shop.show',$productDate->category->id)}}"><i class="far fa-map"></i>{{$productDate->category->category_name}}</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
